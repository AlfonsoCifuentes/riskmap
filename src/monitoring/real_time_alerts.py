...
class RealTimeAlertSystem:
    ...
    def __init__(self, config_path: str = "config/alert_config.json", db_path: str = "data/alerts.db"):
    ...
    def _create_default_alert_rules(self):
        ...
        default_rules = [
            AlertRule(
                ...
                regions=["ukraine"],
                keywords=["military", "attack", "invasion", "missile", "bombing"],
            ...
            AlertRule(
                ...
                severity_threshold="high",
                regions=["*"],  # All regions
                keywords=["cyber", "hacking", "malware", "breach", "espionage"],
            ...
            AlertRule(
                ...
                severity_threshold="critical",
                regions=["*"],
                keywords=["refugee", "disaster", "famine", "epidemic", "evacuation"],
                notification_channels=["email", "websocket"],
                cooldown_minutes=45
            )
        ]
    ...
    async def check_intelligence_report(self, report: Dict[str, Any], article: Dict[str, Any]):
    ...
    def _evaluate_rule_conditions(self, rule: AlertRule, report: Dict[str, Any], article: Dict[str, Any]) -> bool:
        ...
        if rule.regions and "*" not in rule.regions:
            article_regions = [geo.split(':')[0] for geo in report.get("geographic_entities", [])]
        ...
        if rule.keywords:
            ...
            keyword_matches = sum(1 for keyword in rule.keywords if keyword.lower() in text)
    ...
    async def _generate_alert(self, rule: AlertRule, report: Dict[str, Any], article: Dict[str, Any]):
        """Generate and queue alert notification."""
        alert_id = hashlib.md5(f"{rule.rule_id}{article.get('content_hash', '')}{datetime.now()}".encode()).hexdigest()[:16]
        
        # Determine region
        regions = [geo.split(':')[0] for geo in report.get("geographic_entities", [])]
    ...
    def _format_alert_message(self, rule: AlertRule, report: Dict[str, Any], article: Dict[str, Any]) -> str:
...
⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}
    ...
    async def _send_notification(self, notification: AlertNotification):
        ...
        for channel in notification.channels:
            try:
                ...
                logger.info(f"Sent notification {notification.alert_id} via {channel}")
            except Exception as e:
    ...
    async def _send_discord_notification(self, notification: AlertNotification):
