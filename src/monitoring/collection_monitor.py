...
class GlobalCollectionMonitor:
    ...
    def __init__(self):
        ...
        self.monitoring_enabled = config.get('monitoring.collection_monitoring.enabled', True)
        
    def track_collection_performance(self, collection_type: str, source_info: Dict[str, Any], 
                                   articles_collected: int, duration_seconds: float,
        ...
        try:
            ...
            logger.debug(f"Tracked collection performance: {collection_type} - {articles_collected} articles")
        ...
        try:
            ...
            return {
                ...
                'overall': {
                    ...
                    'avg_articles_per_collection': round(overall_stats[2] or 0, 1),
                    'avg_duration_seconds': round(overall_stats[3] or 0, 2),
                    'overall_success_rate': round(overall_stats[4] or 0, 3)
                },
                'by_type': type_stats,
    ...
    def get_source_reliability_report(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                reliability_score = row[6] * (1 - failure_rate)  # Success rate adjusted for failures
            ...
            high_reliability = [s for s in source_reliability if s['reliability_score'] >= 0.8]
            medium_reliability = [s for s in source_reliability if 0.5 <= s['reliability_score'] < 0.8]
            low_reliability = [s for s in source_reliability if s['reliability_score'] < 0.5]
    ...
    def get_coverage_analysis(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            for row in cursor.fetchall():
                try:
                    ...
                    for country in countries:
                        regional_coverage[country] = regional_coverage.get(country, 0) + row[1]
                except (json.JSONDecodeError, TypeError):
                    continue
            
            # Coverage gaps analysis
            expected_languages = config.get('data_sources.global_coverage.priority_languages', [])
            missing_languages = [lang for lang in expected_languages if lang not in language_coverage]
            ...
            coverage_score = min(
                language_diversity / len(expected_languages) if expected_languages else 1.0,
                1.0
            )
            ...
            return {
                ...
                'regional_coverage': dict(sorted(regional_coverage.items(), 
    ...
    def _generate_reliability_recommendations(self, source_reliability: List[Dict]) -> List[str]:
        """Generate recommendations based on source reliability analysis."""
        recommendations = []
        
        # Check for sources with high failure rates
        high_failure_sources = [s for s in source_reliability if s['failure_rate'] > 0.3]
        ...
        low_yield_sources = [s for s in source_reliability if s['avg_articles'] < 1.0 and s['attempts'] > 5]
        ...
        slow_sources = [s for s in source_reliability if s['avg_duration'] > 30.0]
        if slow_sources:
            recommendations.append(
        ...
        underrepresented = [lang for lang, count in language_counts.items() if count < 3]
        if underrepresented:
            recommendations.append(
    ...
    def create_monitoring_tables(self):
        ...
        try:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_collection_timestamp ON collection_performance(timestamp)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_collection_type ON collection_performance(collection_type)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_collection_source ON collection_performance(source_name)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_collection_language ON collection_performance(language)')
...
class SourceHealthChecker:
    ...
    def check_source_health(self, source_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def batch_health_check(self, sources: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for source in sources:
            try:
                ...
                logger.error(f"Error checking source {source.get('name', 'unknown')}: {e}")
                results.append({
