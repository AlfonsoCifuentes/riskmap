...
*_secrets.json
        ...
        logger.info(
            f"[OK] Created .gitignore for key protection at {gitignore_path}")
    ...
    def _get_master_password(self) -> str:
        ...
        if self._master_key is None:
            ...
            if env_password:
                ...
                logger.info(
                    "[OK] Master password loaded from environment variable")
            else:
                ...
                if self.encrypted_file.exists():
                    ...
                    print(
                        "💡 Tip: Set RISKMAP_MASTER_KEY environment variable to avoid prompts")
    ...
    def encrypt_and_save_keys(
            self, api_keys: Dict[str, str], password: str = None) -> bool:
        ...
        try:
            ...
            logger.info(
                f"[OK] {len(api_keys)} API keys encrypted and saved securely")
            print(f"🔒 API keys encrypted and saved to: {self.encrypted_file}")

            # Mostrar claves enmascaradas para confirmación
            print("\n📋 Saved keys (masked):")
            for key_name, key_value in api_keys.items():
                masked_key = f"{key_value[:8]}...{key_value[-4:]}" if len(
                    key_value) > 12 else "***masked***"
    ...
    def load_and_decrypt_keys(self, password: str = None) -> Dict[str, str]:
        ...
        try:
            ...
            logger.info(
                f"[OK] {len(api_keys)} API keys decrypted successfully")
            return api_keys
    ...
    for key_name, key_info in required_keys.items():
        ...
        while True:
            if key_info['required']:
                ...
                if not key_value:
                    print(
                        "[ERROR] This key is required. Please enter a valid value.")
                    continue
            else:
                key_value = getpass.getpass(
                    f"Enter {key_name} (optional, press Enter to skip): ").strip()
                if not key_value:
            ...
            if key_name == 'NEWSAPI_KEY' and len(key_value) < 30:
                ...
                print(
                    "[ERROR] Google Translate key format seems incorrect. Please check.")
                continue
