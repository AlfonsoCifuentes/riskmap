...
from utils.config import config, DatabaseManager, detect_language
...
class MultilingualGeopoliticalChatbot:
    ...
    def __init__(self):
        ...
        self.question_patterns = {
            'regional_stability': [
                'regiones más inestables', 'areas of conflict', 'нестабильные регионы', 
                '不稳定地区', 'المناطق غير المستقرة'
            ],
            'conflict_analysis': [
                'conflictos militares', 'military conflicts', 'военные конфликты', 
                '军事冲突', 'النزاعات العسكرية'
            ],
            'protest_monitoring': [
                'protestas', 'protests', 'протесты', '抗议活动', 'الاحتجاجات'
            ],
            'diplomatic_crisis': [
                'crisis diplomática', 'diplomatic crisis', 'дипломатический кризис',
                '外交危机', 'الأزمة الدبلوماسية'
            ],
            'sentiment_trends': [
                'tendencias sentimiento', 'sentiment trends', 'тенденции настроения',
                '情感趋势', 'اتجاهات المشاعر'
            ]
        }
    ...
    def _initialize_ai_components(self):
        ...
        if not self.openai_key or self.openai_key == "your_openai_key_here":
            logger.warning("[WARN] OpenAI API key no configurada - funcionalidad limitada")
            return
        ...
        try:
            if LANGCHAIN_AVAILABLE:
                ...
                self.embeddings_model = OpenAIEmbeddings(openai_api_key=self.openai_key)
                
                self.ai_available = True
                logger.info("[OK] Componentes de IA inicializados correctamente")
...
"""
    ...
    def process_query(self, query: str, chat_history: List[List[str]] = None) -> str:
        """
        ...
        try:
            ...
            if self.ai_available and self.conversation_chain:
                response = self._generate_langchain_response(query, relevant_articles, detected_lang)
            elif self.openai_key:
                response = self._generate_openai_response(query, relevant_articles, detected_lang)
            else:
                response = self._generate_template_response(query, relevant_articles, detected_lang)
            
            return response
            
        except Exception as e:
            logger.error(f"Error procesando consulta: {e}")
            return self._get_error_message(detected_lang if 'detected_lang' in locals() else 'en')
    
    def _generate_langchain_response(self, query: str, articles: List[Dict], language: str) -> str:
...
"""
    ...
    def _generate_openai_response(self, query: str, articles: List[Dict], language: str) -> str:
    ...
    def _generate_template_response(self, query: str, articles: List[Dict], language: str) -> str:
    ...
    def _get_error_message(self, language: str) -> str:
        ...
        return self.language_responses.get(language, self.language_responses['en'])['error']
            ...
            with gr.Column(scale=4):
                ...
                with gr.Row():
                    ...
                    with gr.Column(scale=1, min_width=100):
                        send_button = gr.Button("[START] Enviar", variant="primary")
