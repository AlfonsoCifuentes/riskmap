...
class MultiModelAIClient:
    ...
    def _get_model_priority(self) -> List[str]:
        ...
        priority_str = os.getenv('AI_MODEL_PRIORITY', 'deepseek,openai,groq,huggingface,local')
    ...
    def _check_available_models(self) -> Dict[str, bool]:
        ...
        available['deepseek'] = bool(deepseek_key and deepseek_key != 'your_deepseek_api_key_here')
        
        # Groq
        groq_key = os.getenv('GROQ_API_KEY')
        available['groq'] = bool(groq_key and groq_key != 'your_groq_api_key_here')
        
        # HuggingFace
        hf_key = os.getenv('HUGGINGFACE_API_KEY')
        available['huggingface'] = bool(hf_key and hf_key != 'your_huggingface_api_key_here')
    ...
    def generate_analysis(self, articles: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        for model_name in self.priority:
            if self.available_models.get(model_name, False):
                try:
                    logger.info(f"Attempting to use {model_name} for AI analysis")
    ...
    def _prepare_articles_text(self, articles: List[Dict[str, Any]]) -> str:
        ...
        return "\n\n".join([
            ...
            for article in articles[:8]  # Limitar para evitar límites de tokens
        ])
...
El artículo debe fluir naturalmente desde una introducción hasta conclusiones, conectando los eventos reportados en un análisis geopolítico coherente."""
    ...
    def _generate_with_model(self, model_name: str, prompt: str) -> Optional[str]:
        ...
        try:
            ...
            for model_name in models_to_try:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if isinstance(result, list) and len(result) > 0:
                            ...
                            if generated_text and len(generated_text) > 50:
                                logger.info(f"HuggingFace success with model: {model_name}")
                                return self._post_process_hf_text(generated_text)
                    ...
                    logger.warning(f"HuggingFace model {model_name} returned: {response.status_code}")
                    
                except Exception as model_error:
                    logger.warning(f"HuggingFace model {model_name} failed: {model_error}")
                    continue
