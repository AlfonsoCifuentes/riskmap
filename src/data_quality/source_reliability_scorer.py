...
class SourceReliabilityScorer:
    ...
    def evaluate_source(self,
        ...
        metrics.content_quality_score = self._evaluate_content_quality(
            source_info, article_samples)
        metrics.factual_accuracy_score = self._evaluate_factual_accuracy(
            source_info, domain)
        metrics.editorial_standards_score = self._evaluate_editorial_standards(
            source_info, domain)
        metrics.transparency_score = self._evaluate_transparency(
            source_info, domain)

        metrics.political_bias_score = self._assess_political_bias(
            source_info, article_samples)
        metrics.sensationalism_score = self._assess_sensationalism(
            source_info, article_samples)
        ...
        metrics.reliability_score = self._calculate_overall_reliability(
            metrics)
    ...
    def _evaluate_content_quality(
            self, source_info: Dict[str, Any], article_samples: List[Dict[str, Any]]) -> float:
        ...
        if domain in self.trusted_sources:
            score = self.trusted_sources[domain] * \
                0.8  # 80% weight for known sources
        ...
        if article_samples:
            ...
            if content_scores:
                ...
                score = (score + avg_content_score) / \
                    2  # Average with base score
    ...
    def _evaluate_factual_accuracy(
            self, source_info: Dict[str, Any], domain: str) -> float:
    ...
    def _evaluate_editorial_standards(
            self, source_info: Dict[str, Any], domain: str) -> float:
    ...
    def _evaluate_transparency(
        ...
        if '.gov' in domain or 'official' in source_info.get(
                'name', '').lower():
            score += 0.2

        return min(1.0, max(0.0, score))

    def _assess_political_bias(
            self, source_info: Dict[str, Any], article_samples: List[Dict[str, Any]]) -> float:
    ...
    def _calculate_overall_reliability(
            self, metrics: SourceReliabilityMetrics) -> float:
        """Calculate overall reliability score."""
        ...
        score = (
            ...
            # Invert sensationalism
            weights['sensationalism'] * (1 - metrics.sensationalism_score) +
    ...
    def _calculate_confidence_interval(
            self, metrics: SourceReliabilityMetrics) -> Tuple[float, float]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """,
                (metrics.source_id,
                 ...
                 metrics.factual_accuracy_score,
                 metrics.editorial_standards_score,
                 metrics.transparency_score,
                 metrics.uptime_percentage,
                 metrics.response_time_avg,
                 metrics.update_frequency,
                 metrics.content_freshness,
                 metrics.political_bias_score,
                 metrics.sensationalism_score,
                 metrics.fact_check_rating,
                 metrics.articles_per_day,
                 metrics.consistency_score,
                 metrics.duplicate_content_ratio,
                 metrics.citation_count,
                 metrics.social_media_engagement,
                 metrics.expert_endorsements,
        ...
        for keyword in self.risk_indicators['suspicious_keywords']:
            if keyword.lower() in name.lower():
                ...
                recommendations.append(
                    "Review content for bias or misinformation")
    ...
    def get_source_rankings(
            self, limit: int = 50, min_reliability: float = 0.5) -> List[Dict[str, Any]]:
    ...
    def get_reliability_report(self) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            total_sources = conn.execute(
                "SELECT COUNT(*) FROM source_metrics").fetchone()[0]
...
if __name__ == "__main__":
    ...
    print(
        f"Reliability score for {test_source['name']}: {metrics.reliability_score:.3f}")

    validation = scorer.validate_source(test_source)
    print(
        f"Validation result: {validation.is_valid}, Risk: {validation.risk_level}")
