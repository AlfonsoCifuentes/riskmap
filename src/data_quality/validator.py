...
class DataValidator:
    ...
    def validate_article(self, article_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_result = self._validate_required_fields(article_data, validation_result)
            
            # Content quality checks
            validation_result = self._validate_content_quality(article_data, validation_result)
            
            # Source credibility check
            validation_result = self._validate_source_credibility(article_data, validation_result)
            
            # Language and encoding validation
            validation_result = self._validate_language_encoding(article_data, validation_result)
            
            # Duplicate detection
            validation_result = self._check_for_duplicates(article_data, validation_result)
            
            # Spam and low-quality content detection
            validation_result = self._detect_spam_content(article_data, validation_result)
    ...
    def _validate_required_fields(self, article: Dict[str, Any], result: Dict[str, Any]) -> Dict[str, Any]:
        """Validate required fields are present and non-empty."""
        required_fields = ['title', 'content', 'url', 'published_at', 'source']
        
        for field in required_fields:
            if field not in article or not article[field]:
                result['issues'].append(f"Missing or empty required field: {field}")
    ...
    def _validate_content_quality(self, article: Dict[str, Any], result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(content) < self.min_article_length:
            result['issues'].append(f"Content too short: {len(content)} characters")
            result['quality_score'] -= 15
        elif len(content) > self.max_article_length:
            result['warnings'].append(f"Very long content: {len(content)} characters")
            result['quality_score'] -= 5
    ...
    def _validate_source_credibility(self, article: Dict[str, Any], result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _validate_language_encoding(self, article: Dict[str, Any], result: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if text_field:
                try:
                    ...
                    if len(normalized) != len(text_field):
                        result['warnings'].append(f"Potential encoding issues in {field_name}")
                        result['quality_score'] -= 3
                    
                    # Check for excessive special characters
                    special_char_ratio = sum(1 for c in text_field if not c.isalnum() and not c.isspace()) / len(text_field)
                    if special_char_ratio > 0.1:
                        result['warnings'].append(f"High special character ratio in {field_name}")
                        result['quality_score'] -= 5
                    
                except Exception:
                    result['warnings'].append(f"Text encoding validation failed for {field_name}")
        
        return result
    
    def _check_for_duplicates(self, article: Dict[str, Any], result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if duplicate:
                result['issues'].append(f"Duplicate content detected (ID: {duplicate[0]})")
    ...
    def _detect_spam_content(self, article: Dict[str, Any], result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if suspicious_found:
            result['warnings'].append(f"Suspicious patterns detected: {suspicious_found}")
        ...
        if content:
            ...
            if len(words) > 20:
                ...
                repeated_phrases = [p for p, count in phrase_counts.items() if count > 3]
                if repeated_phrases:
                    result['warnings'].append("Repeated phrases detected")
                    result['quality_score'] -= min(10, len(repeated_phrases) * 2)
    ...
    def validate_batch(self, articles: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            'validation_rate': round((valid_count / len(articles)) * 100, 2),
    ...
    def cleanup_invalid_articles(self, min_quality_score: int = 30) -> int:
        ...
        try:
            ...
            if count_to_delete > 0:
                ...
                conn.commit()
                logger.info(f"Cleaned up {count_to_delete} low-quality articles")
