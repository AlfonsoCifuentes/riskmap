...
def api_articles():
    ...
    try:
        ...
        count_query = query.split(' LIMIT')[0].replace(
            'a.*, ar.sentiment, ar.risk_level, ar.summary, ar.entities', 'COUNT(*)')
    ...
    try:
        ...
        for language in languages:
            try:
                ...
                logger.info(
                    f"Global RSS collection for {language}: {len(articles)} articles")
            except Exception as e:
        ...
        for language in languages:
            try:
                articles = newsapi_collector.collect_multilingual_headlines(
                    language)
                total_articles += len(articles)
                results[f'newsapi_{language}'] = len(articles)
                logger.info(
                    f"NewsAPI collection for {language}: {len(articles)} articles")
            except Exception as e:
        ...
        if include_intelligence:
            try:
                ...
                logger.info(
                    f"Intelligence collection: {len(intel_articles)} articles")
            except Exception as e:
...
def api_regional_collection(region):
    ...
    try:
        ...
        if region not in valid_regions:
            return jsonify(
                {'error': f'Invalid region. Must be one of: {valid_regions}'}), 400
        ...
        for language in priority_languages:
            try:
                sources = global_registry.get_sources_by_region(
                    language, region)
                if sources:
                    articles = rss_collector.collect_from_sources(
                        sources, max_articles_per_source)
                    total_articles += len(articles)
                    results[f'{region}_{language}'] = len(articles)
                    logger.info(
                        f"Regional collection {region}/{language}: {len(articles)} articles")
            except Exception as e:
                logger.error(
                    f"Regional collection failed for {region}/{language}: {e}")
...
def api_global_sources():
    ...
    try:
        ...
        for lang in global_registry.get_supported_languages():
            ...
            source_info['global_news']['sources_by_language'][lang] = len(
                lang_sources)
...
def run_api_server(host='0.0.0.0', port=5001, debug=False):
    """Run the API server."""
    logger.info(
        f"Starting Geopolitical Intelligence API server on {host}:{port}")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Geopolitical Intelligence System API')
