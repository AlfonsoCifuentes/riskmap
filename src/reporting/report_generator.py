...
try:
    ...
    logging.warning(
        "WeasyPrint not available, using reportlab for PDF generation")
...
try:
    ...
    logging.warning(
        "xhtml2pdf not available, using reportlab for PDF generation")
...
class AdvancedReportGenerator:
    ...
    def __init__(self):
        ...
        self.report_sections = {
            ...
            'entity_tracking': self._generate_entity_tracking,
            'source_reliability': self._generate_source_reliability,
    ...
    def generate_comprehensive_report(
        ...
        if date_range is None:
            ...
            start_date = end_date - \
                timedelta(days=1 if report_type == 'daily' else 7)
        ...
        logger.info(
            f"Generando reporte {report_type} para período {date_range[0]} - {date_range[1]}")
    ...
    def generate_daily_report(
            self, target_date: datetime = None) -> Dict[str, Any]:
        """Genera reporte diario usando la plantilla profesional."""
        if target_date is None:
            target_date = datetime.now()

        logger.info(
            f"Generando reporte diario para {target_date.strftime('%Y-%m-%d')}")
        ...
        output_files = self._save_report_files(
            html_content, filename, daily_data)
    ...
    def generate_executive_report(self,
                                  ...
                                  end_date: datetime = None) -> Dict[str,
                                                                     Any]:
        """Genera reporte ejecutivo usando la plantilla profesional."""
        if end_date is None:
            end_date = datetime.now()
        if start_date is None:
            # Reporte semanal por defecto
            start_date = end_date - timedelta(days=7)

        logger.info(
            f"Generando reporte ejecutivo para período {start_date.strftime('%Y-%m-%d')} - {end_date.strftime('%Y-%m-%d')}")
        ...
        output_files = self._save_report_files(
            html_content, filename, executive_data)
    ...
    def _get_daily_data(self, start_date: datetime,
    ...
    def _get_executive_data(self, start_date: datetime,
                            end_date: datetime) -> Dict[str, Any]:
    ...
    def _save_report_files(self, html_content: str,
                           filename: str, data: Dict = None) -> Dict[str, str]:
        ...
        for format_type in formats:
            ...
            if format_type == 'html':
                output_files[format_type] = self._save_html_report(
                    report_content, file_path)
            elif format_type == 'pdf':
                output_files[format_type] = self._save_pdf_report(
                    report_content, file_path)
    ...
    def generate_weekly_report(
            self, end_date: datetime = None) -> Dict[str, str]:
        ...
        logger.info(
            f"Generating weekly report for {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
        ...
        sections = {}
        enabled_sections = self.report_config.get(
            'sections', [
                'executive_summary', 'risk_assessment', 'regional_analysis', 'trend_analysis'])

        for section_name in enabled_sections:
            if section_name in self.templates['sections']:
                logger.info(f"Generating section: {section_name}")
                sections[section_name] = self.templates['sections'][section_name](
                    data, end_date, start_date)

        # Generate final report
        report_content = self._compile_report(
            sections, data, end_date, 'weekly', start_date)
    ...
    def _collect_daily_data(self, date: datetime) -> Dict[str, Any]:
        ...
        # Top articles by significance (negative sentiment + conflict
        # categories)
        cursor.execute('''
            SELECT
    ...
    def _collect_weekly_data(self, start_date: datetime,
                             end_date: datetime) -> Dict[str, Any]:
    ...
    def _generate_executive_summary(
            self, data: Dict[str, Any], date: datetime, start_date: datetime = None) -> str:
        ...
        if risk_regions:
            avg_risk_sentiment = sum(
                r['avg_sentiment'] for r in risk_regions[:5]) / min(len(risk_regions), 5)
            high_risk_count = sum(
                1 for r in risk_regions if r['avg_sentiment'] < -0.3)
        else:
        ...
        period_text = date.strftime(
            '%Y-%m-%d') if start_date is None else f"{start_date.strftime('%Y-%m-%d')} to {date.strftime('%Y-%m-%d')}"
        ...
        if risk_regions:
            ...
            region_name = ', '.join(
                top_region['countries'][:2]) if top_region['countries'] else 'Unknown'
    ...
    def _generate_risk_assessment(self,
        ...
        for region in risk_regions[:10]:
            region_name = ', '.join(
                region['countries'][:2]) if region['countries'] else 'Unknown'
        ...
        for category in categories:
            ...
            if category['category'] in [
                'military_conflict',
                    'diplomatic_crisis'] and sentiment < -0.2:
                risk_indicator = '🔴 High'
            elif sentiment < -0.1:
    ...
    def _generate_regional_analysis(
            self, data: Dict[str, Any], date: datetime, start_date: datetime = None) -> str:
        """Generate regional analysis section."""
        regional_events = data.get(
            'regional_events',
            []) or data.get(
            'risk_regions',
            [])
        ...
        if not regional_events:
            ...
            for area, regions in regions_by_area.items():
                ...
                for region in regions[:5]:  # Top 5 per area
                    ...
                    sentiment_text = "positive" if sentiment > 0.1 else "negative" if sentiment < - \
                        0.1 else "neutral"
    ...
    def _generate_geopolitical_trends(
            self, data: Dict[str, Any], date: datetime, start_date: datetime = None) -> str:
    ...
    def _group_regions_by_area(
            self, regional_events: List[Dict]) -> Dict[str, List[Dict]]:
    ...
    def _compile_report(self,
        ...
        significant_events = data.get(
            'significant_events', []) or data.get(
            'significant_articles', [])
        events_html = ""
        ...
        if significant_events:
            ...
            for event in significant_events[:10]:
                countries_text = ', '.join(
                    event.get(
                        'countries', [])[
                        :3]) if event.get('countries') else 'Global'
                sentiment_class = 'negative' if event.get(
                    'sentiment', 0) < -0.1 else 'neutral'
    ...
    def _save_pdf_report(self, content: str, file_path: Path) -> str:
        ...
        try:
            ...
            if WEASYPRINT_AVAILABLE:
                ...
                simple_data = {
                    'executive_summary': 'Reporte generado desde HTML'}
...
<body>
    ...
    def _generate_sentiment_analysis(self, data: Dict[str, Any], date: datetime, start_date: datetime = None) -> str:
        ...
        html = "<div class='sentiment-analysis'><h2>Sentiment Analysis</h2><table><tr><th>Label</th><th>Avg Score</th><th>Count</th></tr>"
        for avg, label, count in sentiments:
            html += f"<tr><td>{label}</td><td>{avg:.2f if avg else 0.00}</td><td>{count}</td></tr>"
        html += "</table></div>"
        conn.close()
        return html

    def _generate_entity_tracking(self, data: Dict[str, Any], date: datetime, start_date: datetime = None) -> str:
        """Generate entity tracking section."""
        conn = self.db.get_connection()
        cursor = conn.cursor()
        start = start_date if start_date else date - timedelta(days=1)
        ...
        locations = Counter()
        for p, l in entities:
            if p: persons.update([person.strip() for person in p.split(',')])
            if l: locations.update([loc.strip() for loc in l.split(',')])
        html = "<div class='entity-tracking'><h2>Entity Tracking</h2><h3>Top Persons</h3><ul>"
        for person, count in persons.most_common(5):
            html += f"<li>{person}: {count}</li>"
        html += "</ul><h3>Top Locations</h3><ul>"
    ...
    def _generate_source_reliability(self, data: Dict[str, Any], date: datetime, start_date: datetime = None) -> str:
        """Generate source reliability section."""
        conn = self.db.get_connection()
        cursor = conn.cursor()
        start = start_date if start_date else date - timedelta(days=1)
        ...
        html = "<div class='source-reliability'><h2>Source Reliability</h2><table><tr><th>Source</th><th>Count</th><th>Reliability</th></tr>"
        for source, count in sources:
            reliability = 'High'  # Placeholder
            html += f"<tr><td>{source}</td><td>{count}</td><td>{reliability}</td></tr>"
        html += "</table></div>"
        conn.close()
        return html

    def _generate_recommendations(self, data: Dict[str, Any], date: datetime, start_date: datetime = None) -> str:
        """Generate recommendations section."""
        html = "<div class='recommendations'><h2>Recommendations</h2><ul><li>Monitor high-risk regions closely.</li><li>Prepare contingency plans for potential escalations.</li><li>Conduct further analysis on emerging trends.</li></ul></div>"
        return html
...
def main():
    ...
    def _generate_simple_pdf_report(self, data: Dict, pdf_path: Path) -> None:
        ...
        try:
            ...
            story.append(
                Paragraph(
                    "Reporte de Inteligencia Geopolítica",
                    title_style))
            ...
            if data and 'featured_events' in data and data['featured_events']:
                ...
                for i, event in enumerate(
                        data['featured_events'][:5]):  # Primeros 5 eventos
                    story.append(
                        Paragraph(
                            f"{i+1}. {event.get('summary', 'Sin resumen')}",
                            styles['Normal']))
                    story.append(
                        Paragraph(
                            f"   País: {event.get('region', 'N/A')} | Categoría: {event.get('category', 'N/A')}",
                            styles['Italic']))
