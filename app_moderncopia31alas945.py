...
try:
    ...
    print("[WARNING] Algunos módulos no están disponibles. El sistema continuará con funcionalidad limitada.")
    print("[INFO] Para funcionalidad completa, ejecuta manualmente: pip install -r requirements_enhanced_historical_fixed.txt")
    
    # Continue without some components
    GeopoliticalIntelligenceOrchestrator = None
    TaskScheduler = None
    EnhancedHistoricalOrchestrator = None
    HistoricalDashboard = None
    MultivariateRelationshipDashboard = None
...
class RiskMapUnifiedApplication:
    ...
    def _setup_api_endpoints(self):
        ...
        try:
            if self.config['enable_api'] and self.core_orchestrator and create_api_blueprint is not None:
                ...
                logger.warning("API endpoints not available - create_api_blueprint not loaded")
        except Exception as e:
    ...
    def _initialize_all_systems(self):
        ...
        try:
            ...
            if GeopoliticalIntelligenceOrchestrator is not None:
                ...
                logger.warning("Core orchestrator not available - skipping initialization")
            ...
            if EnhancedHistoricalOrchestrator is not None:
                ...
                logger.warning("Enhanced historical orchestrator not available - skipping initialization")
            
            # 4. Initialize task scheduler
            if TaskScheduler is not None and self.core_orchestrator is not None:
                logger.info("Initializing task scheduler...")
                self.task_scheduler = TaskScheduler(self.core_orchestrator)
            else:
                logger.warning("Task scheduler not available - skipping initialization")
            ...
            logger.info("All available systems initialized successfully with existing data loaded")
