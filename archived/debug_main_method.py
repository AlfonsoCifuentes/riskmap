#!/usr/bin/env python3
"""
Test the EXACT backend path to understand why it's using fallback
"""
import sqlite3
import os
import logging
...
def debug_main_method():
    print("🔍 DEBUGGING MAIN METHOD STEP BY STEP")
    print("="*60)
    
    limit = 20
    exclude_hero_id = None
    
    # EXACT COPY of main method logic from app_BUENA.py
    print("Step 1: Getting database path...")
    try:
    ...
    if not os.path.exists(db_path):
        ...
        print("   Would use _get_real_articles_from_db() fallback")
        return
    else:
    ...
    print(f"✅ Query built with exclude_clause: {exclude_clause}")
    ...
    try:
        ...
        print(f"✅ Query executed successfully: {len(rows)} results")
        ...
        if rows:
            ...
            print("❌ No results returned - would use fallback")
