...
from src.utils.config import config, db_manager, logger as config_logger
...
class RiskMapUnifiedApplication:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self._update_statistics_from_db()  # Actualizar estadísticas desde BD
        self._setup_flask_routes()
        self._setup_signal_handlers()
        
    def _update_statistics_from_db(self):
        """Actualiza las estadísticas del sistema desde la base de datos real"""
        try:
            ...
            # Artículos de alto riesgo (como alertas)
            ...
            logger.info(f"📊 Estadísticas actualizadas: {total_articles:,} artículos, {processed_articles:,} procesados, {risk_alerts:,} alertas de riesgo")
            
        except Exception as e:
            logger.error(f"❌ Error actualizando estadísticas desde BD: {e}")
    ...
    def _setup_flask_routes(self):
        ...
        def get_articles():
            """Obtiene lista de artículos desde la base de datos real"""
            try:
                ...
                logger.info(f"📰 Obtenidos {len(articles)} artículos de la base de datos (página {page})")
                ...
                logger.error(f"❌ Error al obtener artículos de la base de datos: {e}")
        ...
        @self.flask_app.route('/api/database/stats')
        def get_database_stats():
            ...
            try:
                ...
                # Top 10 países con más artículos
                cursor.execute("""
                ...
                cursor.execute("""
                    ...
                    ORDER BY date DESC
                """)
                stats['articles_by_date'] = dict(cursor.fetchall())
                
                # Estadísticas de contenido
                cursor.execute("SELECT COUNT(*) FROM articles WHERE sentiment_score IS NOT NULL")
                stats['articles_with_sentiment'] = cursor.fetchone()[0]
                ...
                return jsonify({'error': 'Error al obtener estadísticas de base de datos'}), 500
    ...
    def _get_test_articles(self) -> List[Dict]:
        """Obtiene artículos reales de la base de datos para análisis"""
        try:
            conn = db_manager.get_connection()
            cursor = conn.cursor()
            
            # Obtener una muestra diversa de artículos recientes
            cursor.execute("""
                SELECT id, title, content, country as location, risk_level, 
                       risk_score, source, created_at, sentiment_score, summary
                FROM articles 
            ...
            for row in rows:
                article = dict(zip(column_names, row))
                # Asegurar que risk_score sea numérico
                if article.get('risk_score') is None:
                    article['risk_score'] = 0.5  # Valor por defecto
                articles.append(article)
            
            conn.close()
            
            if articles:
                logger.info(f"📰 Obtenidos {len(articles)} artículos reales para análisis")
                return articles
            else:
                logger.warning("⚠️ No hay artículos en la base de datos, usando datos de fallback")
                # Fallback a datos de prueba si no hay artículos en BD
                return self._get_fallback_articles()
                
        except Exception as e:
            logger.error(f"❌ Error obteniendo artículos de BD: {e}")
            return self._get_fallback_articles()
    
    def _get_fallback_articles(self) -> List[Dict]:
        """Artículos de fallback cuando no hay datos en BD"""
        return [
            {
                'id': 1,
