...
class AIImageGenerator:
    ...
    def analyze_article_for_image_prompt(
            self,
            article_content: str,
        ...
        try:
            ...
            try:
                ...
                if image_prompt:
                    ...
                    self.logger.info(
                        f"Generated image prompt: {enhanced_prompt[:100]}...")
                    return enhanced_prompt

            except ImportError:
                self.logger.warning(
                    "AI models not available, using fallback prompt generation")
    ...
    def generate_image_from_prompt(
        ...
        try:
            # Clean filename
            safe_filename = "".join(
                c for c in filename if c.isalnum() or c in (
                    ' ', '-', '_')).rstrip()
            image_path = os.path.join(
                self.images_dir,
                f"{safe_filename}_{int(time.time())}.png")
    ...
    def generate_image_for_analysis(
            self,
            analysis_content: str,
            analysis_title: str = "Geopolitical Analysis") -> Optional[str]:
        """
        ...
        try:
            ...
            image_prompt = self.analyze_article_for_image_prompt(
                analysis_content, analysis_title)

            # Step 2: Generate image from prompt
            filename = f"analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            image_path = self.generate_image_from_prompt(
                image_prompt, filename)

            if image_path:
                # Return relative path for web serving
                relative_path = image_path.replace(
                    '\\', '/').replace('data/', '/static/data/')
                self.logger.info(
                    f"Image generation completed: {relative_path}")
                return relative_path
            else:
                self.logger.warning("Image generation failed, using default")
                return None
        ...
        try:
            ...
            for filename in os.listdir(self.images_dir):
                ...
                if os.path.isfile(file_path):
                    file_age_hours = (
                        current_time - os.path.getmtime(file_path)) / 3600
