...
class AdvancedSystemMonitor:
    ...
    def check_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Puntuación final
            health_status['score'] = total_score // len(
                checks) if checks else 0
    ...
    def _process_alerts(self, health_status: Dict[str, Any]) -> None:
        ...
        if health_status['overall_status'] == 'critical':
            logger.critical(
                f"ALERTA CRÍTICA: Sistema en estado crítico (Score: {health_status['score']})")
        elif health_status['overall_status'] == 'warning':
            logger.warning(
                f"ALERTA: Sistema con advertencias (Score: {health_status['score']})")

        # Alertas específicas
        for check_name, check_result in health_status['checks'].items():
            if check_result.get('status') == 'critical':
                logger.critical(
                    f"CRÍTICO - {check_name}: {check_result.get('message', 'Sin detalles')}")
            elif check_result.get('status') == 'warning':
                logger.warning(
                    f"ADVERTENCIA - {check_name}: {check_result.get('message', 'Sin detalles')}")
    ...
    def _check_system_resources(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'status': status, 'message': '; '.join(
                messages) if messages else 'Recursos del sistema normales', 'metrics': metrics}
        ...
        try:
            ...
            for log_file in log_files:
                try:
                    ...
                    error_count = sum(
                        1 for line in lines if 'ERROR' in line.upper())
                    warning_count = sum(
                        1 for line in lines if 'WARNING' in line.upper())
        ...
        try:
            ...
            for report_file in self.log_dir.glob("health_check_*.json"):
                if datetime.fromtimestamp(
                        report_file.stat().st_mtime) < cutoff_date:
                    report_file.unlink()
                    cleaned_files += 1
    ...
    def _check_network_connectivity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check internet access by pinging Google DNS
...
SystemMonitor = AdvancedSystemMonitor
