...
class AIChat:
    ...
    def _initialize_model(self):
        ...
        try:
            # Try to use HuggingFace transformers (completely free and
            # open-source)
            from transformers import pipeline, AutoTokenizer, AutoModelForCausalLM
            import torch

            # Use a lightweight model that works well in Spanish and for
            # analysis
            ...
            logger.warning(
                "Transformers library not available, using rule-based responses")
            self.model_initialized = False
...
Respuesta del analista:"""
    ...
    def _clean_ai_response(self, response: str) -> str:
        ...
        if response and not response.endswith(('.', '!', '?', ':')):
            ...
            if last_period > len(response) * \
                    0.5:  # If we have at least half the response
    ...
    def _generate_intelligent_fallback(self, user_message: str) -> str:
        ...
        if "conflicto" in message_lower or "guerra" in message_lower:
            return self._generate_conflict_analysis(
            ...
            return self._generate_regional_analysis(
                user_message, relevant_context)
        elif "económico" in message_lower or "comercio" in message_lower:
            return self._generate_economic_analysis(
                user_message, relevant_context)
...
Las tendencias se actualizan continuamente basándose en análisis de big data y machine learning."""
    ...
    def _generate_general_response(
        ...
        responses = [f"Tu consulta sobre '{user_message}' toca aspectos importantes del análisis geopolítico moderno. En el contexto internacional actual, es fundamental considerar la interconexión entre factores políticos, económicos, sociales y de seguridad para comprender completamente las dinámicas globales.",
